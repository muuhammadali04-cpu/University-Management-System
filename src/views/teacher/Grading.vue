...
const getStudentGrades = (studentId) => {
  if (!selectedClass.value) return { midterm: 0, final: 0, assignments: 0 }
  const record = gradesStore.records.find(r => r.student_id === studentId && r.subject_id === selectedClass.value.subject_id)
  return record ? { midterm: record.midterm, final: record.final, assignments: record.assignments } : { midterm: 0, final: 0, assignments: 0 }
}
...
const initGrades = () => {
  ...
  classStudents.value.forEach(student => {
    gradeForm.value[student.id] = getStudentGrades(student.id)
  })
}
  ...
  <div class="flex flex-col h-full gap-8 relative">
    ...
    <div v-else class="flex flex-col h-full bg-white/70 backdrop-blur-md rounded-3xl border border-white/60 shadow-xl overflow-hidden">
      ...
      <div class="px-8 py-6 border-b border-slate-200/60 bg-gradient-to-r from-slate-50 to-white flex justify-between items-center">
        <div>
          <button @click="selectedClass = null" class="text-sm font-bold text-primary hover:text-primary-dark mb-2 flex items-center gap-1 transition-colors">
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="19" y1="12" x2="5" y2="12"></line><polyline points="12 19 5 12 12 5"></polyline></svg>
            Back to Classes
          </button>
          <h2 class="text-2xl font-black text-slate-800">{{ selectedClass.subject?.name }} <span class="text-slate-400 font-medium">({{ selectedClass.section?.name }})</span></h2>
        </div>
      </div>
      
      
      <form @submit.prevent="saveGrades" class="p-8 flex-1 flex flex-col overflow-hidden">
        <div class="flex justify-between items-center mb-6">
          <h3 class="font-bold text-slate-700 text-lg">Student Grading</h3>
          <span class="text-xs font-bold text-slate-400 bg-slate-100 px-3 py-1 rounded-full uppercase tracking-wider">Total: 100 Marks</span>
        </div>
          ...
          <table class="w-full text-left border-collapse">
            <thead>
              <tr class="border-b border-slate-200">
                ...
                <th class="py-3 px-4 text-xs uppercase tracking-wider font-bold text-slate-500 w-32">Midterm (30)</th>
                <th class="py-3 px-4 text-xs uppercase tracking-wider font-bold text-slate-500 w-32">Final (50)</th>
                <th class="py-3 px-4 text-xs uppercase tracking-wider font-bold text-slate-500 w-32">Asgmts (20)</th>
                <th class="py-3 px-4 text-xs uppercase tracking-wider font-bold text-slate-500 w-24">Total</th>
              </tr>
            </thead>
            <tbody>
              <tr v-for="student in classStudents" :key="student.id" class="border-b border-slate-100 hover:bg-slate-50/50 transition-colors">
                ...
                <td class="py-4 px-4">
                  <input type="number" v-model.number="gradeForm[student.id].midterm" min="0" max="30" class="form-input shadow-sm text-center px-2 py-1.5" required />
                </td>
                <td class="py-4 px-4">
                  <input type="number" v-model.number="gradeForm[student.id].final" min="0" max="50" class="form-input shadow-sm text-center px-2 py-1.5" required />
                </td>
                <td class="py-4 px-4">
                  <input type="number" v-model.number="gradeForm[student.id].assignments" min="0" max="20" class="form-input shadow-sm text-center px-2 py-1.5" required />
                </td>
                <td class="py-4 px-4 font-black text-slate-700">
                  {{ (Number(gradeForm[student.id].midterm) || 0) + (Number(gradeForm[student.id].final) || 0) + (Number(gradeForm[student.id].assignments) || 0) }}
                </td>
              </tr>
              <tr v-if="classStudents.length === 0">
                <td colspan="5" class="py-8 text-center text-slate-500 font-medium">No students enrolled in this section.</td>
              </tr>
            </tbody>
          </table>
        </div>
        <div class="pt-6 border-t border-slate-200 flex justify-end">
          <button type="submit" class="btn-primary shadow-lg shadow-primary/20 px-8">Save All Grades</button>
        </div>
      </form>
    </div>
  </div>
